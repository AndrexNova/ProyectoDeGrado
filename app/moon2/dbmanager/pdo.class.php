...
class Moon2_DBmanager_PDO {
    ...
    protected function get_where($arrSearch, $other = "") {
        ...
        if (!array_key_exists(null, $arrSearch)) {
            ...
            //echo "<br />";
            //echo "otro --->";
            //print_r($this->_arrSearch);
            //echo "<br />la llave ----->".$key;
...
//pendiente para ajuste y actualizacion
    ...
    public function add_programa($Obj) {
        $fields = "";
        $Params = array();
        $set_method = "set";
        $get_method = "get";
        ...
        $Ro = new ReflectionObject($Obj);
        $properties = $Ro->getProperties();
        $cons = "SELECT max({$this->_Pkey["key"]}) as {$this->_Pkey["key"]} FROM {$this->_table} where " . $this->_Pkey["key"] . " != ?";
        $Arr = $this->GetRow($cons, array(''));
...
//            
            if ($this->_Pkey["key"] != $name) {
                ...
                $fields.=$this->_Pkey["key"] . ",";
                $max = $Arr[$this->_Pkey["key"]] + 1;
                $Params[] = str_pad($max, 3, 0, STR_PAD_LEFT);                
            }
        }      
...
//        exit();
    ...
    public function add_materia($Obj) {
        $fields = "";
        $Params = array();
        $set_method = "set";
        $get_method = "get";
        
        $method = $get_method . "_" . "id_programa";
        $r = $Obj->$method();
        $cons = "SELECT max({$this->_Pkey["key"]}) as {$this->_Pkey["key"]} FROM {$this->_table} WHERE id_materia like '$r%' and id_materia != ?";
        $Arr = $this->GetRow($cons, array(''));
...
//        var_dump($Arr);
        ...
        foreach ($properties as $property) {
            ...
            if ($this->_Pkey["key"] != $name) {
                ...
                $fields.=$this->_Pkey["key"] . ",";
                $max = $Arr[$this->_Pkey["key"]] + 1;
                $Params[] = str_pad($max, 6, 0, STR_PAD_LEFT);                
            }
        }
