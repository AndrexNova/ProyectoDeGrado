    ...
    switch($this->_type){
        ...
        case "Radio":
                foreach($this->_arrValues as $key => $value){
                        ...
                        $selected = $this->checkSelected($key);
                        if ($selected) {$check = " checked=\"checked\"";}
                        if ($this->_style === "BootstrapSwitch"){
                            $xhtml.= "<div class=\"radio i-checks\"><label>\n";
                            $xhtml.=" <input type=\"radio\" name=\"{$this->_fieldName}\" id=\"{$this->_id}\" value=\"{$key}\"{$check} {$this->_options} {$this->_style}/>&nbsp;{$value}<br />\n";
                            $xhtml.= "</label></div>\n";
                        }else{
                            $xhtml.="<input type=\"radio\" name=\"{$this->_fieldName}\" id=\"{$this->_id}\" value=\"{$key}\"{$check} {$this->_options} {$this->_style}/>&nbsp;{$value}<br />\n";
                        }
        ...
        break;
         case "RadioHorizontal":
                foreach($this->_arrValues as $key => $value){
                        ...
                        $selected = $this->checkSelected($key);
                        if ($selected) $check = " checked=\"checked\"";
                        $xhtml.="<input type=\"radio\" name=\"{$this->_fieldName}\" id=\"{$this->_id}\" value=\"{$key}\"{$check} {$this->_options} {$this->_style}/>&nbsp;{$value}\n";
                }
        break;
    }
    return $xhtml;
