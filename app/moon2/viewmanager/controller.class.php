...
public function headerTable1($dataArray, $order, $Parameters){
    $tmp_params = clone $Parameters;
    $xhtml = "<thead>\n";
    $xhtml.= "<tr class=\"primary \">\n";
    foreach ($dataArray as $key => $value){
        if (!empty($value["name"])){
            $icon = " ";
            $tmp_params->add("order",$key);
            ...
            $xhtml.= "<a href=\"".$_SERVER['PHP_SELF']."?".$url_params."\">{$icon}"."<label class=\"col-xs-6 lead\">".$value["name"]."</label>"."</a>";
            $xhtml.= "</th>\n";
        }else{
            $xhtml.= "<th>&nbsp;</th>\n";
        }
...
public function headerTable2($dataArray, $order, $Parameters){
    $tmp_params = clone $Parameters;
    $xhtml = "<thead>\n";
    $xhtml.= "<tr class=\"primary \">\n";
    foreach ($dataArray as $key => $value){
...
public function buildMenu($parent, $mainPage){
    ...
    $html= "";
    $html.= "<li{$active}>\n";
    $html.= "  <a href=\"{$mainPage}/views\"><i class=\"fa fa-windows\"></i><span class=\"nav-label\">Inicio</span></a>\n";
    $html.= "</li>\n";
    ...
    foreach ($arrayData as $key => $vector){
        if ($vector["codpadre"] == $parent){
            ...
            $html.= "<li{$active }>\n";
            $html.= "  <a href=\"#\"><i class=\"{$vector["icono"]}\"></i><span class=\"nav-label\">{$vector["nombre"]}</span><span class=\"fa arrow\"></span></a>\n";
                if ($vector["hijos"] > 0){
...
public function buildNavigationbar(){
    ...
    if ($total>0){
        $last = $total - 1;
        $xhtml.= "<div class=\"row wrapper border-bottom white-bg\">\n";
        $xhtml.= "  <div class=\"col-lg-10\">\n";
        $xhtml.= "    <ol class=\"breadcrumb\">\n";
        for ($i=0; $i<$total; $i++){
            if ($i==$last){
                $xhtml.= "<li class=\"active\"><strong>".$this->_navigationBar[$i]["text"]."</strong></li>\n";
            }else{
