...
class Modules_Krauff_ModelDb_usuariosdb extends Moon2_DBmanager_PDO {
    ...
    public function validate($user, $pass) {
        ...
        $sql="SELECT u.codusuario, u.codperfil, u.nombreusuario, (u.nombres || ' ' || u.primerapellido || ' ' || u.segundoapellido) as nombrecompleto,";
        $sql.="p.nombreperfil FROM {$this->_table} u INNER JOIN perfiles p ON u.codperfil = p.codperfil ";
    ...
    public function load_all_admin(&$rsNumRows, $limit_numrows, $page, $Data = array()) {
        $Fields = "p.nombreperfil, u.codusuario, u.genero, (u.nombres||' '||u.primerapellido||' '||u.segundoapellido) AS nombres, u.tipodoc, u.documento, u.nombreusuario ";
        $From = "FROM {$this->_table} u";
        $where = $this->get_where($Data["search"]);
        $order = " ORDER BY " . $Data["order"] . " ASC";
    ...
    public function load_all_clientes(&$rsNumRows, $limit_numrows, $page, $Data = array()) {
        ...
        if (empty($caja_campos) && empty($caja_busqueda)) {
            ...
            $sql = "SELECT count(*) ";
            $sql.="FROM usuarios u INNER JOIN perfiles p ON u.codperfil = p.codperfil INNER JOIN empresas e ON u.codempresa = e.codempresa ";
            $sql.="WHERE u.codempresa = {$this->_DOM["EMPRESA_ID"]} and u.codperfil = 4 and u.codusuario <> -2 ";
            $rsNumRows = $this->GetOne($sql);
            return $arr;
        } else {
            $where = $this->get_where($Data["search"]);
            ...
            $sql = "SELECT count(*) ";
            $sql.="FROM usuarios u INNER JOIN perfiles p ON u.codperfil = p.codperfil INNER JOIN empresas e ON u.codempresa = e.codempresa ";
            $sql.=$where . " AND e.codempresa = {$this->_DOM["EMPRESA_ID"]} and u.codperfil = 4 ";
            $rsNumRows = $this->GetOne($sql);
            return $arr;
        }
    ...
    $sql = "SELECT u.codusuario, u.nombres || ' ' || u.primerapellido || ' '  ||u.segundoapellido ";
    $sql.= "FROM usuarios u ORDER BY u.nombres";
 ...
 public function buscar_codusuario($nombres) {
        $parametros = array($nombres);
